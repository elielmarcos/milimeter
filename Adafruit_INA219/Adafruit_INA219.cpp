...
Adafruit_INA219::Adafruit_INA219(uint8_t addr) {
  ina219_i2caddr = addr;
  ina219_currentMultiplier_mA = 0.0f;
  ina219_powerMultiplier_mW = 0.0f;
  ina219_BUSMultiplier_V = 0.0f;
  ina219_shuntMultiplier_mV = 0.0f;
}
...
float Adafruit_INA219::getShuntVoltage_mV() {
  ...
  return (value * ina219_shuntMultiplier_mV);
}
...
float Adafruit_INA219::getBusVoltage_V() {
  ...
  return (value * ina219_BUSMultiplier_V);
}
...
float Adafruit_INA219::getCurrent_mA() {
  ...
  return (value * ina219_currentMultiplier_mA);
}
...
void Adafruit_INA219::setCalibration_32V_2A() {
  ...
  ina219_currentMultiplier_mA = 0.1f; 		// Current LSB = 100uA per bit
  ina219_powerMultiplier_mW = 1.0f; 		// Power LSB = 1mW per bit (2/1)
  ina219_BUSMultiplier_V = 0.004f;          // Bus_Volt LSB = 4mV per bit
  ina219_shuntMultiplier_mV = 0.01f;        // Shunt_Volt LSB = 10uV per bit
  ...
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_32V |
                    INA219_CONFIG_GAIN_8_320MV | INA219_CONFIG_BADCRES_12BIT_16S_8510US |
...
void Adafruit_INA219::setCalibration_32V_1A() {
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentMultiplier_mA = 0.04f; 		// Current LSB = 40uA per bit
  ina219_powerMultiplier_mW = 0.8f; 		// Power LSB = 800uW per bit (2/1)
  ina219_BUSMultiplier_V = 0.004f;          // Bus_Volt LSB = 4mV per bit
  ina219_shuntMultiplier_mV = 0.01f;        // Shunt_Volt LSB = 10uV per bit
 ...
 *     current measurement (0.02mA), at the expense of
 *     only supporting 16V at 400mA max.
 */
void Adafruit_INA219::setCalibration_16V_400mA(uint32_t calValue) {
	
  // EQ. 1	
  // CAL = trunc(0.04096 / (Current_LSB x RSHUNT) )
  ...
  // EQ. 4
  // Current_Reg = ( Shunt_Volt_Reg x CAL ) / 4096
  
  // EQ. 5
  // Power_Reg = ( Current_Reg x BUS_Volt_Reg ) / 5000
  
  // EQ. 6
  // Corrected_Full_Scale_Cal = trunc(( CAL x MeasShuntCurrent ) / INA219_Current )
  ...
  // VBUS_MAX = 16V
  // GAIN = 1
  // Max_ShuntVoltage = 40mV (Full Scale Register - FSR)
  // ------------
  // Load = 10A
  // Vcm = 12V
  // Voltage Differential (Vshunt) = 20mV
  // ------------
  // MinLSB => EQ. 2 = 15A / 2^15 = 457.78uA/bit
  // Current_LSB = Rounded MinLSB = 1mA/bit
  // CAL => EQ. 1 = trunc(0.04096 / (0.001 x 0.002) ) = 20480
  // ------------
  // Shifted right 3bit => BUS_Volt_Reg = 2995 (BB3h)
  // BUS_Volt = (Vcm - Vshunt) = BUS_Volt_Reg x 4mV = 2995 x 0.004 = 11.98V
  //
  // => Current_Reg => EQ. 4 = ( Shunt_Volt_Reg x CAL ) / 4096 = 10000 (2710h)
  // Current = Current_Reg x Current_LSB = 10000 x 0.001 = 10A
  //
  // => Power_Reg => EQ. 5 = ( Current_Reg x BUS_Volt_Reg ) / 5000 = 5990 (1766h)
  // Power = Power_LSB x Power_Reg = ( 20 x Current_LSB ) x Power_Reg = (20 x 0.001) x 5990 = 119.8W
  ...
  //
  // Max_Current_Before_Overflow = MaxPossible_I
  // Max_Current_Before_Overflow = 0.4A
  //
  // Max_ShuntVoltage = Max_Current_Before_Overflow * RSHUNT
  ...
  // Set multipliers to convert raw current/power/BUS values
  ina219_currentMultiplier_mA = 0.02f;      // Current LSB = 20uA per bit 
  ina219_powerMultiplier_mW = 0.4f;         // Power LSB = 0.4mW per bit
  ina219_BUSMultiplier_V = 0.004f;          // Bus_Volt LSB = 4mV per bit
  ina219_shuntMultiplier_mV = 0.01f;        // Shunt_Volt LSB = 10uV per bit
  ...
  // Set Config register to take into account the settings above (BUS 16V,
  // GAIN 1/40mV, BUS ADC 12bits 32Samples, SHUNT ADC 12bits 32Samples, BUS and
  // SHUNT Continuous)
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_1_40MV | INA219_CONFIG_BADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_SADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS;
 ...
 *          of current.  Each unit of current corresponds to 25uA, and
 *          each unit of power corresponds to 0.5mW. Counter overflow
 *          occurs at 0.8A.
 *  @note   These calculations assume a 0.1 ohm resistor is present
 */
void Adafruit_INA219::setCalibration_16V_800mA(uint32_t calValue) {
  // By default we use a pretty huge range for the input voltage,
  // which probably isn't the most appropriate choice for system
  // that don't use a lot of power.  But all of the calculations
  // are shown below if you want to change the settings.  You will
  ...
  // VSHUNT_MAX = 0.08          (Assumes Gain 2, 800mA)
  // RSHUNT = 0.1               (Resistor value in ohms)
  ...
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0,0001923             (192uA per bit)
  ...
  // Set multipliers to convert raw current/power values  
  ina219_currentMultiplier_mA = 0.025f; 	// Current LSB = 25uA per bit
  ...
  // Set Config register to take into account the settings above (BUS 16V,
  // GAIN 2/80mV, BUS ADC 12bits 32Samples, SHUNT ADC 12bits 32Samples, BUS and
  // SHUNT Continuous)
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_2_80MV | INA219_CONFIG_BADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_SADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS;
 ...
 *     only supporting 16V at 1600A max.
 */
void Adafruit_INA219::setCalibration_16V_1600mA(uint32_t calValue) {

  // Calibration which uses the highest precision for
  // current measurement (0.05mA), at the expense of
  // only supporting 16V at 1600mA max.
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.0000488              (49uA per bit)
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.0003906              (390uA per bit)
  ...
  //
  // Max_Current_Before_Overflow = MaxPossible_I
  // Max_Current_Before_Overflow = 1.6A
  //
  // Max_ShuntVoltage = Max_Current_Before_Overflow * RSHUNT
  ...
  //
  // Max_ShuntVoltage_Before_Overflow = VSHUNT_MAX
  // Max_ShuntVoltage_Before_Overflow = 0.16V
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentMultiplier_mA = 0.05f;      // Current LSB = 50uA per bit 
  ...
  // Set Config register to take into account the settings above (BUS 16V,
  // GAIN 4/160mV, BUS ADC 12bits 32Samples, SHUNT ADC 12bits 32Samples, BUS and
  // SHUNT Continuous)
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_4_160MV | INA219_CONFIG_BADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_SADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS;
 ...
 *     only supporting 16V at 3200A max.
 */
void Adafruit_INA219::setCalibration_16V_3200mA(uint32_t calValue) {
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.0000976              (98uA per bit)
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.0007812              (781uA per bit)
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentMultiplier_mA = 0.1f;      // Current LSB = 100uA per bit 
  ina219_powerMultiplier_mW = 2.0f;         // Power LSB = 2mW per bit
  ina219_BUSMultiplier_V = 0.004f;          // Bus_Volt LSB = 4mV per bit
  ina219_shuntMultiplier_mV = 0.01f;        // Shunt_Volt LSB = 10uV per bit
  ...
  // Set Config register to take into account the settings above (BUS 16V,
  // GAIN 8/320mV, BUS ADC 12bits 32Samples, SHUNT ADC 12bits 32Samples, BUS and
  // SHUNT Continuous)
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_8_320MV | INA219_CONFIG_BADCRES_12BIT_16S_8510US |
                    INA219_CONFIG_SADCRES_12BIT_16S_8510US |
